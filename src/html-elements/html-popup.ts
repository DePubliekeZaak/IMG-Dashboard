...
export class HtmlPopup {
  ...
  pop(el) {
    ...
    if (window.innerWidth > 700) {
      ...
      this.popupElement.style.top =
        (44 + rect.top + window.scrollY).toString() + "px";
  ...
  formule() {
    ...
    top.innerHTML = `
      ...
      + ( <span title="besluiten immateriele_schade" class="top value ims_besluiten"></span> * <span title="doorlopend rapportcijfer ims" class="top value w_cijfer"></span> )
      + ( <span title="besluiten vaste_vergoeding" class="top value ves_besluiten"></span> * <span title="doorlopend rapportcijfer ves" class="top value w_cijfer"></span> )
  ...
  populateFormula() {
    ...
    let score =
      (this.data[0].waardedaling_besluiten *
        ...
        this.data[0].fysieke_schade_besluiten *
          this.data[0].fysieke_schade_doorlopend_cijfer) /
      (this.data[0].waardedaling_besluiten +
        this.data[0].fysieke_schade_besluiten);
    ...
    (
      document.querySelector(
        "#img-dashboard_popup .top.value.ims_besluiten",
      ) as HTMLElement
    ).innerText = this.data[0].immateriele_schade_besluiten;
    (
      document.querySelector(
        "#img-dashboard_popup .bottom.value.ims_besluiten",
      ) as HTMLElement
    ).innerText = this.data[0].immateriele_schade_besluiten;
    (
      document.querySelector(
        "#img-dashboard_popup .top.value.ims_cijfer",
      ) as HTMLElement
    ).innerText = this.data[0].immateriele_schade_doorlopend_cijfer;

    (
      document.querySelector(
        "#img-dashboard_popup .top.value.ves_besluiten",
      ) as HTMLElement
    ).innerText = this.data[0].vaste_vergoeding_besluiten;

    (
      document.querySelector(
        "#img-dashboard_popup .bottom.value.ves_besluiten",
      ) as HTMLElement
    ).innerText = this.data[0].vaste_vergoeding_besluiten;

    (
      document.querySelector(
        "#img-dashboard_popup .top.value.ves_cijfer",
      ) as HTMLElement
    ).innerText = this.data[0].vaste_vergoeding_doorlopend_cijfer;

    (
      document.querySelector("#img-dashboard_popup .kto_result") as HTMLElement
    ).innerText = (Math.round(score * 100) / 100).toString();
  }
}
