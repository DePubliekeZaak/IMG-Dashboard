...
export class ChartBarHorizontal {
  ...
  redraw() {
    ...
    this.bars
      ...
      .duration(200)
      .attr("width", (d) =>
        self.ctrlr.scales.x.scale(d[self.ctrlr.parameters.x]),
      );
      ...
      .attr("transform", function (d) {
        let offset =
          window.innerWidth > breakpoints.bax
            ? -(self.ctrlr.scales.y.scale.bandwidth() * 0.15)
            : 0;
        return (
          "translate(" +
          -self.ctrlr.config.padding.left +
          "," +
          (self.ctrlr.scales.y.scale(d[self.ctrlr.parameters.y]) +
            (self.ctrlr.scales.y.scale.bandwidth() + offset)) +
          ")"
        );
      });
    ...
    this.barValues
      .text(function (d) {
        ...
        if (d.no_respondents) {
          let avg =
            (
              Math.round(
                10 * 100 * (d[self.ctrlr.parameters.x] / d.no_respondents),
              ) / 10
            ).toString() + "%";
          text = text + " (" + avg + ")";
        }
      ...
      .attr("transform", function (d) {
        ...
        return (
          "translate(" +
          (self.ctrlr.scales.x.scale(d[self.ctrlr.parameters.x]) + 6) +
          "," +
          (self.ctrlr.scales.y.scale(d[self.ctrlr.parameters.y]) +
            (self.ctrlr.scales.y.scale.bandwidth() + offset)) +
          ")"
