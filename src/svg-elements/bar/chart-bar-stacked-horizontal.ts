...
export class ChartBarStackedHorizontal {
  ...
  draw(data, stackedData) {
    this.series = this.svg.layers.data
      .selectAll(".stackedGroup")
      .data(stackedData);
    ...
    this.barGroup = this.seriesEnter
      .merge(this.series)
      .selectAll("g")
      .data(function (d) {
        return d;
    ...
    this.barGroupEnter = this.barGroup.enter().append("g");
  ...
  redraw(dimensions, xScale, yScale, no_respondents) {
    ...
    this.barLabel
      ...
      .attr("transform", function (d) {
        let offset =
          window.innerWidth > breakpoints.bax ? -(yScale.bandwidth() * 1) : 0;

        return (
          "translate(" +
          0 +
          "," +
          (yScale(d.data[self.config.yParameter]) +
            (yScale.bandwidth() + offset)) +
          ")"
        );
      });
    //
