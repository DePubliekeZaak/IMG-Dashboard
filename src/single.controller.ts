...
export class InitSingle {
  ...
  init() {
    ...
    this.elements = [].slice.call(
      document.querySelectorAll("[data-img-graph-preset]"),
    );

    for (let el of this.elements) {
      el.innerHTML = "";
      this.graphObjectArray.push(
        configs.find(
          (c) => c.slug === el.getAttribute("data-img-graph-preset"),
        ),
      );
    }
  ...
  makeSingleCall(segment, update) {
    ...
    let uniqueEndpoints = [
      ...Array.from(new Set(this.graphObjectArray.map((o) => o.endpoint))),
    ];

    for (let endpoint of uniqueEndpoints) {
      const graphObjectArray = this.graphObjectArray.filter(
        (o) => o.endpoint === endpoint,
      );

      let url =
        endpoint === "/api/gemeenten"
          ? "https://tcmg-hub.publikaan.nl/api/gemeenten"
          : (endpoint || "/api/data") + "?gemeente=eq." + segment;

      d3.json<ResponseData>(url)
        .then((data) => {
          for (let graphObject of graphObjectArray) {
            let containerElement = document.querySelector(
              '[data-img-graph-preset="' + graphObject.slug + '"]',
            );

            for (
              var j = 0;
              j < Object.values(graphObject.mapping).length;
              j++
            ) {
              let map = {};
              map[Object.entries(graphObject.mapping)[j][0]] = Object.entries(
                graphObject.mapping,
              )[j][1];

              // dit of met data
              let element = containerElement.querySelector(
                ".img_graph_container:nth-of-type(" + (j + 1) + ")",
              );
              ...
              if (update && endpoint === "/api/data") {
                ...
                element.innerHTML = "";
                this.graphMethods[j] = new graphs[graphObject.config.graphType](
                  data,
                  element,
                  graphObject.config,
                  Object.values(map)[0],
            ...
            if (
              graphObject.config.extra &&
              graphObject.config.extra.muniSelect
            ) {
