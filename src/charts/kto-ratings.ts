...
import {
  ...
  groupByMonths,
} from "../d3-services/data-with-history.functions";
...
export default class KTORatings extends GraphControllerV2 {
  ...
  init() {
    ...
    this.htmlPeriodSelector = new HtmlPeriodSelector(
      this.element,
      this.mapping.slug,
    ); // later koppelen aan GraphObject.slug
    let column = Array.isArray(this.mapping.parameters[1][1].column)
      ? this.mapping.parameters[1][1].column[0]
      : this.mapping.parameters[1][1].column;
    const completeMonths = getCompleteMonths(this.data).filter(
      (m) => m[column] !== undefined && m[column] !== null,
    );
  ...
  prepareData(data: DataPart[]): GraphData {
    ...
    let selectedMonth = monthIndex
      ? history.find((m) => m["_yearmonth"] === parseInt(monthIndex))
      : history[0];

    if (this.mapping.parameters[1]) {
      // @ts-ignore
      for (let mapping of this.mapping.parameters[dataIndex]) {
        let column = Array.isArray(mapping.column)
          ? mapping.column[0]
          : mapping.column;
        ...
        if (this.mapping.args[0]) {
          cijfer["no_respondents"] =
            dataIndex == 1
              ? selectedMonth[this.mapping.args[0]]
              : selectedMonth[this.mapping.args[1]];
        }
  ...
  redraw(data: GraphData) {
    ...
    let parameter =
      this.segment === "all"
        ? this.mapping.parameters[0][0].column
        : this.mapping.parameters[0][1].column;
    // @ts-ignore
    let extraParameter =
      this.segment === "all"
        ? this.mapping.parameters[0][2].column
        : this.mapping.parameters[0][3].column;
  ...
  draw(data: GraphData) {
    ...
    const periodSelect = document.querySelector(
      ".period_select_" + this.mapping.slug,
