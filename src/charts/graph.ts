...
export class GraphController implements IGraphController  {
    ...
    _svg(svgWrapper?: HTMLElement) {
        ...
        if (this.graphObject.config.scales) {
            for (let c of this.graphObject.config.scales) {
                this.scales[c.slug] = new ChartScale(c.type, this.graphObject.config,this.dimensions)
            }
        } else {
    ...
    redraw(data: any) {
        ...
        if (this.graphObject.config.scales) {
            for (let c of this.graphObject.config.scales) {
                ...
                // klasse en instance gelijk trekken? 
                this.scales[c.slug].reset(c.direction, this.dimensions,undefined,data)
            }
        } else {
