...
export default class BandBars extends GraphControllerV2 {
  ...
  init() {
    ...
    if (
      ...
      this.mapping.args &&
      this.mapping.args[0] === "alternateTicks"
    ) {
    ...
    container.style.height =
      window.innerWidth < breakpoints.sm ? "320px" : "320px";
    ...
    if (
      this.data[0][this.firstMapping["column"]] != null &&
      this.data[0][this.firstMapping["column"]] != 0
    ) {
  ...
  redraw(data: GraphData) {
    this.yScale = this.scales.y.set(
      data.slice.map((d) => d[this.parameters.y]),
    );
