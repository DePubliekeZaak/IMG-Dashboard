...
export default class ChartFocusV2 {
    ...
    redraw(data : GraphData) {
        ...
        function mousemove(event) {
            ...
            let j = data.slice.length - i;
        
            const selectedData = data.slice[j];
            const prevData = (j > 0) ? data.slice[j - 1] : data.slice[0];
            ...
            self.xLine
                ...
                .attr("width", () => {
                    ...
                    if (self.ctrlr.config.scales.find( s => s.slug == 'x').type == 'time' ) {
                       ...
                       return  self.ctrlr.scales.x.scale(new Date(selectedData[self.ctrlr.parameters.x])) - self.ctrlr.scales.x.scale(new Date(prevData[self.ctrlr.parameters.x])) 
        ...
        for (let prop of self.ctrlr.mapping.parameters[0]) {
            ...
            switch (prop.column) {
                ...
                default:
                    ...
                    let value = prop.format === "currency" ? convertToCurrency(d[prop.column]) : d[prop.column];

                    html = html + '<div class="">' + prop.short + ': ' + value + '</div>';
            }
        }
